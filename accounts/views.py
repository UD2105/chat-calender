...
class RegisterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if serializer.validated_data['password'] != request.data['password_confirmation']: # パスワードと確認パスワードが一致しない場合
                return Response({'error': 2}, status=HTTP_400_BAD_REQUEST)
            
            if User.objects.filter(user_id=serializer.validated_data['user_id']).exists(): # UserIDがすでに使われていた場合
                return Response({'error': 3}, status=HTTP_400_BAD_REQUEST)
            
            try:
                serializer.save() # エラーなし
            except:
                return Response({'error': 11}, status=HTTP_500_INTERNAL_SERVER_ERROR) # データベースエラー
